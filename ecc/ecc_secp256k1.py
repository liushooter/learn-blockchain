...
#(Gx**3+7) % Pcurve == (Gy**2) % Pcurve, GPoint 在椭圆曲线上，x/y 坐标符合椭圆曲线方程

h = 1 # Subgroup cofactor, 子群辅因子为 1, 就不参与运算了

# Pcurve, N, GPoint, secp256k1 的函数式，都是严格规定的，严禁修改 !!!
...
class ECC256k1:
    ...
    def EccMultiply(self, GenPoint, ScalarHex): # Double & Add. Not true multiplication
        ...
        Q = GenPoint
        for i in range (1, len(ScalarBin)): # EC 乘法转为标量乘法进行计算 减少运算量
            Q = self.ECdouble(Q)
